...
> **Статус (2026-08-26): выполнено.** Задачи 1–5 реализованы: CORS — один
> слой (`src/middleware.ts` + `utils/allowed-origin.ts`, `utils/cors.ts`
> удалён); auth унифицирован (`requireAuth`, секрет в `src/lib/jwt.ts` без
> дефолта); zod-валидация (`src/schemas/`, `middlewares/validate.ts`);
> формат ответа (`utils/response.ts`); слой сервисов (`src/services/`).
> Остаток по валидации — BACKLOG.md, B-2.
